...
module.exports = function(globalOptions) {
  ...
  if (globalOptions.overrideProtoypeFunctions !== true) {
    _ref = ['applyGraphRelationships', 'removeNode', 'shortestPathTo', 'allRelationshipsBetween', 'incomingRelationshipsFrom', 'outgoingRelationshipsTo', 'removeRelationships', 'removeRelationshipsBetween', 'removeRelationshipsFrom', 'removeRelationshipsTo', 'outgoingRelationships', 'incomingRelationships', 'allRelationships', 'queryRelationships', 'queryGraph', 'createRelationshipBetween', 'createRelationshipFrom', 'createRelationshipTo', 'getNodeId', 'findOrCreateCorrespondingNode', 'findCorrespondingNode', 'dataForNode', 'indexGraph'];
  ...
  Document.prototype.queryRelationships = function(typeOfRelationship, options, cb) {
    ...
    if (options.processPart == null) {
      options.processPart = 'r';
    }
    if (options.returnStatement == null) {
      options.returnStatement = options.processPart;
    }
    if (options.referenceDocumentID == null) {
      options.referenceDocumentID = this._id;
    }
    if (options.endNodeId == null) {
      options.endNodeId = '';
    }
    ...
    return this.getNode(function(nodeErr, fromNode) {
      var cypher, _ref2, _ref3, _ref4, _ref5;
      if (nodeErr) {
      ...
      cypher = _s.sprintf(cypher, {
        ...
        whereRelationship: ((_ref2 = options.where) != null ? _ref2.relationship : void 0) ? "WHERE " + options.where.relationship : '',
  ...
  Document.prototype.findCorrespondingNode = function(options, cb) {
    ...
    id = processtools.getObjectIDAsString(doc);
    if (options.doCreateIfNotExists == null) {
      options.doCreateIfNotExists = false;
    }
    if (options.forceCreation == null) {
    ...
    if (doc.isNew === true && options.forceCreation !== true) {
      ...
      return node.save(function(errSave, node) {
        if (errSave) {
          ...
          return node.index(collectionName, '_id', id, function() {
            return _processNode(node, doc, cb);
          });
        }
      });
  ...
  Document.prototype.allRelationshipsBetween = function(to, typeOfRelationship, options, cb) {
    ...
    return to.getNode(function(err, endNode) {
      if (!endNode) {
        return cb(Error('-> toDocument has no corresponding node', null));
      }
      options.endNodeId = endNode.id;
  ...
  Document.prototype.removeNode = function(options, cb) {
    ...
    if (options.includeRelationships == null) {
  ...
  Document.prototype.indexGraph = function(options, cb) {
    ...
    if (!node) {
      return cb(Error('No node attached'), null);
    }
    if (!(index.length > 0)) {
      return cb(Error('No field(s) to index'), null);
    }
    join = Join.create();
    collectionName = doc.constructor.collection.name;
  ...
  Document.prototype.applyGraphRelationships = function(options, cb) {
    var doc, sortedRelationships, typeOfRelationship, _finally, _ref1;
    _ref1 = processtools.sortOptionsAndCallback(options, cb), options = _ref1.options, cb = _ref1.cb;
    if (!this.schema.get('graphability')) {
      return cb(Error('No graphability enabled'), null);
    }
    if (options.doPersist == null) {
    ...
    return doc.getNode(options, function(err, node, options) {
      ...
      return doc.allRelationships(typeOfRelationship, options, function(err, relationships, options) {
        var collectionName, conditions, data, key, part, relation, update, _id, _j, _k, _len1, _len2, _name, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7;
        if (err) {
        ...
        if ((relationships != null ? relationships.length : void 0) > 0) {
          for (_j = 0, _len1 = relationships.length; _j < _len1; _j++) {
            ...
            if ((_ref2 = relation._data) != null ? _ref2.type : void 0) {
              data = {};
              _ref3 = ['from', 'to'];
              for (_k = 0, _len2 = _ref3.length; _k < _len2; _k++) {
                part = _ref3[_k];
                _ref4 = processtools.extractCollectionAndId(relation.data["_" + part]), collectionName = _ref4.collectionName, _id = _ref4._id;
                data[part] = {
                  collection: collectionName,
                  _id: processtools.ObjectId(_id)
                };
              }
              if (sortedRelationships[_name = relation._data.type] == null) {
  ...
  _queryGraphDB = function(cypher, options, cb) {
    ...
    return graphdb.query(cypher, null, function(errGraph, map) {
      var data, result, _ref3, _ref4, _ref5;
      if (options.debug === true) {
        options.debug = {};
      }
      if ((_ref3 = options.debug) != null) {
...
/*
//@ sourceMappingURL=extendDocument.map
*/
